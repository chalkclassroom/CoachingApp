...
interface Props {
  ...
  sessionId: string
  teacherId: string,
  literacyType: 'Foundational' | 'Writing' | 'Reading' | 'Language'
}
            ...
            {LiteracyCategories[this.props.literacyType].map((value, index) => {
              ...
              return(
                this.state.categoryView === index+1 ? (
                  <DataQuestions
                    key={index}
                    questions={Constants.CoachingQuestions.Literacy[this.props.literacyType][value as LiteracyFoundationalKey | LiteracyReadingKey]}
