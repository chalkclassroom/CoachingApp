...
type sampleDataKey = 'literacy1' | 'literacy2' | 'literacy3' | 'literacy4' | 'literacy5' | 'literacy6' | 'literacy7' | 'literacy8' | 'literacy9' | 'total';
          ...
          <Grid item>
            ...
            >
              {activitySettings.map((value, index) => {
                return (
                  <MenuItem key={index} disabled={index>0 && !childData.some(e => e.activitySetting === value)} onClick={(): void => {handleClose(index)}}>
                  ...
                  {childData.filter(obj => {
                    ...
                    return (
                      <TableCell key={index2} width="20%" align='center' style={{backgroundColor: row.backgroundColor, width: '20%'}}>
                        {view === 'number' ? literacy : (Math.round((literacy/childData[index2].total)*100))+'%'}
                      </TableCell>
                    );
                  })}
                </TableRow>
