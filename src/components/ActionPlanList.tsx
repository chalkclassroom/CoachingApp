 ...
 */
function stableSort(
    array: Array<TeacherListInfo> | Array<ActionPlanInfo>,
    comparator: typeof getComparator
): any {
    const stabilizedThis: Array<Array<
        TeacherListInfo | ActionPlanInfo,
        number
...
class ActionPlanList extends React.Component<Props, State> {
    ...
    handleRequestSort = (
        event: React.SyntheticEvent,
        property: TeacherListInfoKey | ActionPlanInfoKey
    ): void => {
        const isAsc =
        ...
        if (!this.props.actionPlans) {
            firebase
                ...
                .then((answer: Array<TeacherListInfo>) => {
                    answer.forEach((actionPlan: TeacherListInfo) => {
                        firebase
                            .getTeacherFirstName(actionPlan.teacherId)
                            .then((firstName: string) => {
                                actionPlan.teacherFirstName = firstName
                            })
                            .then(() => {
                                firebase
                                    .getTeacherLastName(actionPlan.teacherId)
    ...
    handleCheck = (id: string, practice: string, modified: Date): void => {
        ...
        if (this.props.addActionPlanAttachment && this.props.teacherId) {
            this.props.addActionPlanAttachment(
                id,
                this.props.teacherId,
                (practice === 'AC' ? 'Associative and Cooperative' : practice) +
                    ' ' +
        ...
        return (
            <TableRow key={index} selected={isItemSelected}>
                ...
                >
                    <ListItem
                        onClick={(): void => {
                            this.handleCheck(row.id, row.practice, row.modified)
                        }}
                    >
                        <ListItemIcon>
                            <Checkbox
                                checked={
                                    this.props.checkedActionPlans
                                        ? this.props.checkedActionPlans.includes(
                                              row.id
                                          )
                                        : false
                                }
    ...
    renderActionPlanRow(row: ActionPlanRow, index: number): React.ReactNode {
        ...
        return (
            <TableRow
                ...
                onClick={(): void => {
                    this.props.onClick(row.id, row.teacherId)
                }}
            >
    ...
    renderBody(): React.ReactNode {
        ...
        if (this.props.actionPlans) {
            plans = this.props.actionPlans
        } else {
            plans = this.state.result
        }
        ...
        return (
            stableSort(
                ...
                .map((row: ActionPlanRow, index: number) => {
                    if (this.props.actionPlans) {
                        return this.renderActionPlanAttachmentRow(row, index)
                    } else {
                        return this.renderActionPlanRow(row, index)
                    }
                })
        )
    }
        ...
        return (
            <TableContainer>
                ...
                <TablePagination
                    ...
                    component="div"
                    count={
                        this.props.actionPlans
                            ? this.props.actionPlans.length
