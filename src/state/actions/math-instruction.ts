...
export const deleteMICenters = (): {type: string} => ({
  type: DELETE_MI_CENTERS
});

export const updateMathCount = (behavior: string): {type: string, behavior: string} => ({
  type: UPDATE_MATH_COUNT,
  behavior
})

export const clearMathCount = (): {type: string} => ({
