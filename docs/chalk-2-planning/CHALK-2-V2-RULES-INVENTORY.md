...
Messaging, reports, account settings, CSV import, and export scheduling remain preview-only or delegated until an explicit backend path and rule row are added here. Unknown message/report writes are denied by fallback. Program/site writes are admin-only in the working ruleset.
...
2. Seed fixtures from `.chalk/CHALK-2-V2-RULES-FIXTURES.md`.
