            ...
            .then(async entries => {
                await entries.forEach(async entry => {
                    ...
                    if (entryData.Type === "UNDO") {
                        ...
                        const sqlQuery = `SELECT COUNT(*) as count FROM ${functions.config().env.bq_project}.${functions.config().env.bq_dataset}.test WHERE id = '${row.json.id}' AND sessionStart = TIMESTAMP_SECONDS(${row.json.sessionStart}) AND sessionEnd = TIMESTAMP_SECONDS(${row.json.sessionEnd}) AND teacher = '${row.json.teacher}' AND observedBy = '${row.json.observedBy}' AND activitySetting = '${row.json.activitySetting}' AND checklist.item1 = ${row.json.checklist.item1} AND checklist.item2 = ${row.json.checklist.item2} AND checklist.item3 = ${row.json.checklist.item3} AND checklist.item4 = ${row.json.checklist.item4} AND checklist.item5 = ${row.json.checklist.item5} AND checklist.item6 = ${row.json.checklist.item6} AND checklist.item7 = ${row.json.checklist.item7} AND checklist.item8 = ${row.json.checklist.item8} AND checklist.item9 = ${row.json.checklist.item9} AND checklist.item10 = ${row.json.checklist.item10} AND checklist.item11 = ${row.json.checklist.item11} AND time = TIMESTAMP_SECONDS(${row.json.time})`;
                        const options = {
                            query: sqlQuery,
                            location: 'US',
                        };
