 ...
 */
exports.fetchSiteProfileAverages = functions.https.onCall(
  async (data, context) => {
    ...
    console.log('Data teacherId : ' + data.teacherIds)
    console.log('Data teacherId Length : ' + data.teacherIds.length)
    ...
    //var sqlQuery = `select * from ${functions.config().env.bq_project}.${functions.config().env.bq_dataset}.${observationType}_results `;
    ...
    //sqlQuery += `) and timestamp <= '${endDate}' and timestamp >= '${startDate}'`;
    ...
    if (observationType == 'sequential') {
      sqlQuery = `SELECT
                      ...
                      peopletype,
                      COUNT (sessionStart) AS total,
                      FORMAT_DATETIME("%b-%Y", timestamp) as timestamp  FROM ${
                        functions.config().env.bq_project
                      }.${functions.config().env.bq_dataset}.${observationType}
                      where (${teacherSqlQuery}) and sessionStart <= '${endDate}' and sessionStart >= '${startDate}'
                      GROUP BY teacher, timestamp, peopletype, startDate
                      ORDER BY timestamp ASC;`
    }
    ...
    if (observationType == 'literacyFoundationalTeacher') {
      sqlQuery = `SELECT FORMAT_DATE('%D', DATE(sessionStart)) AS startDate,
                      ...
                      COUNT(CASE WHEN (checklist.item10) THEN 'foundational10' ELSE NULL END) AS foundational10,
                      COUNT(CASE WHEN (checklist.item11) THEN 'foundational11' ELSE NULL END) AS foundational11,
                      COUNT (sessionStart) AS total,
                      teacher,
                      time
                      FROM ${functions.config().env.bq_project}.${
    ...
    if (observationType == 'ac') {
      sqlQuery = `SELECT FORMAT_DATE('%D', DATE(sessionStart)) AS startDate,
                      ...
                      COUNT(CASE WHEN (checklist.child5) THEN 'child5' ELSE NULL END) AS childNoInteraction,
                      COUNT(CASE WHEN (peopleType = 1 OR peopleType = 2) THEN 'noOpportunity' ELSE NULL END) AS noOpportunity,
